...
IJ.run("Image Sequence...", "open="+parentPath+"/EdU sort");
imp3 = WindowManager.getImage("EdU"); // EdU image
...
bufferedWriter.write("## ROI\tSlice\tEdU\tX\tY\tCy5\tFociPerCell\tCondition\n");
def bufferedWriter2 = new BufferedWriter(new FileWriter(parentPath+"/SimpleFociList.txt"));
bufferedWriter2.write("## ROI\tSlice\tEdU\tFociPerCell\tFociPerCellNoise\tCondition\n");
for(int i=0; i<rois.size(); i++){
	...
	// Measure EdU intensity for each ROI
	imp3.setPosition(slice);
	ImageProcessor ip3 = imp3.getProcessor();
	ip3.setRoi(rois[i]);
	edu = ip3.getStats().mean;
	bufferedWriter2.write((i+1)+"\t"+slice+"\t"+edu+"\t"+points.size()+"\t"+addNoise(points.size())+"\t"+conditions[slice-1]+"\n");
	
	// For each maxima point, measure Cy5 focus intensity
	imp.setPosition(slice);
	ImageProcessor ip = imp.getProcessor();
	
	points.each{
		cy5 = ip.getPixel(it.x.asType(int), it.y.asType(int));
		bufferedWriter.write((i+1)+"\t"+slice+"\t"+edu+"\t"+it.x+"\t"+it.y+"\t"+cy5+"\t"+points.size()+"\t"+conditions[slice-1]+"\n");
	}
}
