...
var solifity_threshold=0.90;

// Use DAPI image sequence
run("8-bit");
...
for(i=0; i<roiManager("count"); i++){
	roiManager("select", i);
	 ...
	 if(isCell(a, s, bx, by, bw, bh, area_threshold)){	 	
...
function isCell(a, s, bx, by, bw, bh, area_threshold){
	...
	if(s<solifity_threshold) return false;
	return true;
}
